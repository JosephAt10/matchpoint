...
<head>
    ...
    <title>{{ __('Browse Fields') }} - MatchPoint</title>
...
<body class="min-h-screen bg-[linear-gradient(180deg,#ffffff_0%,#f7f4ff_42%,#ffffff_100%)] text-ink">
            ...
            <nav class="flex items-center gap-3 text-[16px] font-medium text-[#5d6385]">
                ...
                @auth
                    <form action="{{ route('logout') }}" method="POST" class="inline-flex">
                        @csrf
                        <button type="submit" class="px-4 py-3 transition hover:text-indigoDeep">{{ __('Logout') }}</button>
                    </form>
                @else
        ...
        <section class="relative overflow-hidden border-b border-[#ece7ff] bg-[linear-gradient(180deg,#ffffff_0%,#f7f4ff_86%,#f5f1ff_100%)] px-8 pb-8 pt-8 lg:px-14 2xl:px-20">
            <div class="grid gap-4 lg:grid-cols-[0.86fr_1.14fr] lg:items-end">
                <div class="relative z-10 max-w-[560px] pt-6">
                    <p class="text-[16px] font-semibold uppercase tracking-[0.26em] text-orangeAccent">{{ __('Field Search') }}</p>
                    <h1 class="mt-4 font-heading text-[42px] font-bold leading-[1.05] tracking-tight text-ink lg:text-[50px] 2xl:text-[56px]">
                        {{ __('Find the perfect field') }}
                        <br>
                        {{ __('for :game', ['game' => __('your game.')]) }}
                    </h1>
                    <p class="mt-5 max-w-[520px] text-[18px] leading-9 text-copy">
                        {{ __('Browse approved sports venues and book the perfect place to play, compete, and have fun.') }}
                    </p>
                </div>

                <div class="relative min-h-[250px] lg:min-h-[330px] 2xl:min-h-[380px]">
                    <img src="{{ asset('landing/field-top-right-image.png') }}" alt="{{ __('Sports equipment and stadium') }}" class="absolute inset-0 h-full w-full object-contain object-right-bottom">
                </div>
            </div>
            ...
            <form method="GET" action="{{ route('fields.index') }}" class="relative z-10 mt-2 grid gap-5 rounded-[2rem] border border-white/80 bg-white/95 p-6 shadow-panel lg:grid-cols-[1fr_1fr_1fr_220px_160px] lg:items-end">
                <label class="space-y-3">
                    <span class="block text-[15px] font-medium text-copy">{{ __('Location') }}</span>
                    <div class="flex items-center gap-3 rounded-2xl border border-[#e6e0ff] bg-white px-4 py-4">
                        <svg class="h-5 w-5 text-[#7a76a8]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s-6-4.35-6-10a6 6 0 1112 0c0 5.65-6 10-6 10z"/><circle cx="12" cy="11" r="2.5"></circle></svg>
                ...
                <label class="space-y-3">
                    <span class="block text-[15px] font-medium text-copy">{{ __('Sport type') }}</span>
                    <div class="flex items-center gap-3 rounded-2xl border border-[#e6e0ff] bg-white px-4 py-4">
                        <svg class="h-5 w-5 text-[#7a76a8]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="8"></circle><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16M4 12h16M7.5 7.5c2 1.2 4.4 1.8 4.5 4.5-.1 2.7-2.5 3.3-4.5 4.5m9-9c-2 1.2-4.4 1.8-4.5 4.5.1 2.7 2.5 3.3 4.5 4.5"/></svg>
                ...
                <label class="space-y-3">
                    <span class="block text-[15px] font-medium text-copy">{{ __('Field type') }}</span>
                    <div class="flex items-center gap-3 rounded-2xl border border-[#e6e0ff] bg-white px-4 py-4">
                        <svg class="h-5 w-5 text-[#7a76a8]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="6" width="18" height="12" rx="2"></rect><path stroke-linecap="round" stroke-linejoin="round" d="M9 6v12m6-12v12M3 12h18"/></svg>
                        <select name="type" class="w-full border-0 bg-transparent p-0 text-[18px] text-[#50567d] outline-none">
                            <option value="">{{ __('Indoor and outdoor') }}</option>
                            <option value="Indoor" @selected(($filters['type'] ?? '') === 'Indoor')>{{ __('Indoor') }}</option>
                            <option value="Outdoor" @selected(($filters['type'] ?? '') === 'Outdoor')>{{ __('Outdoor') }}</option>
                        </select>
                    </div>
                </label>
                ...
                <button type="submit" class="flex items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-[#5d55df] to-[#6b63f4] px-6 py-4 text-[18px] font-semibold text-white shadow-[0_18px_34px_rgba(99,89,237,0.24)] transition hover:opacity-95">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M7 12h10M10 18h4"/></svg>
                    {{ __('Apply filters') }}
                </button>
                ...
                <a href="{{ route('fields.index') }}" class="flex items-center justify-center gap-3 rounded-2xl border border-[#e6e0ff] bg-white px-6 py-4 text-[18px] font-medium text-[#5f6488] transition hover:bg-mist">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v6h6M20 20v-6h-6"/><path stroke-linecap="round" stroke-linejoin="round" d="M20 9a8 8 0 00-13.66-5.66L4 10m16 4l-2.34 2.66A8 8 0 014 15"/></svg>
                    {{ __('Reset') }}
                </a>
            </form>
        </section>
        ...
        <section class="mx-auto mt-10 w-full max-w-[1820px] px-8 pb-12 lg:px-14 2xl:px-20">
            <div class="mb-6 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="font-heading text-[34px] font-bold text-ink">{{ __('Popular Sports Venues') }}</h2>
                </div>
                <a href="{{ route('fields.index') }}" class="flex items-center gap-2 text-[20px] font-medium text-indigoDeep transition hover:opacity-80">
                    {{ __('View all') }}
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14M13 5l7 7-7 7"/></svg>
                </a>
            </div>
            ...
            <div class="mt-8 flex justify-center">
                @if ($fields->hasMorePages())
                    <button id="load-more-button" type="button" data-next-page="{{ $fields->nextPageUrl() }}" class="inline-flex items-center gap-3 rounded-[1.3rem] border-2 border-[#7568f8] bg-white px-10 py-4 text-[20px] font-semibold text-indigoDeep transition hover:bg-[#f7f4ff]">
                        {{ __('Load more venues') }}
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m0 0l6-6m-6 6l-6-6"/></svg>
                    </button>
                @else
    ...
    <script>
        ...
        const loadMoreText = @json(__('Load more venues'));
        const loadMoreError = @json(__('No fields matched your filters.'));
