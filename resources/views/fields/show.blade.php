@php
    ...
    $displayDate = $selectedDate->translatedFormat('M j, Y');
...
<head>
    ...
    <script>
        tailwind.config = { theme: { extend: { colors: { ink: '#151a3b', indigoSoft: '#6b63f6', indigoDeep: '#5542d9', copy: '#676d92' }, fontFamily: { heading: ['Outfit', 'sans-serif'], body: ['DM Sans', 'sans-serif'] } } } }
    </script>
    <style>
        * { font-family: 'DM Sans', sans-serif; }
        h1, h2, h3, h4, h5, h6, .font-heading { font-family: 'Outfit', sans-serif; }
        .booking-date-input::-webkit-calendar-picker-indicator { cursor: pointer; opacity: 0; }
    </style>
</head>
<body class="min-h-screen bg-[linear-gradient(180deg,#ffffff_0%,#f8f6ff_50%,#ffffff_100%)] text-ink">
    ...
    <main class="w-full px-5 py-8 md:px-10 xl:px-16 2xl:px-24">
        ...
        @if ($errors->any())<div class="mb-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-800">{{ $errors->first() }}</div>@endif
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_520px] 2xl:grid-cols-[minmax(0,1fr)_560px] xl:items-start">
            <div class="space-y-6">
                <article class="overflow-hidden rounded-xl border border-[#ebe7fb] bg-white shadow-[0_18px_44px_rgba(86,75,165,0.10)]">
                    <div class="relative h-[250px] overflow-hidden md:h-[315px]">
                        ...
                        <span class="absolute left-5 top-5 rounded-xl bg-white px-5 py-3 text-[14px] font-bold text-ink shadow-[0_10px_26px_rgba(21,26,59,0.12)]">{{ __($field->type) }}</span>
                    </div>
                        ...
                        <div class="flex flex-col gap-5 border-b border-[#eceaf5] pb-7 md:flex-row md:items-center md:justify-between">
                            <div class="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <p class="mt-3 flex items-center gap-2 text-[14px] font-medium text-[#363b5f]"><svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s-6-4.35-6-10a6 6 0 1112 0c0 5.65-6 10-6 10z"/><circle cx="12" cy="11" r="2.5"></circle></svg>{{ $field->location }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4 md:justify-end">
                                ...
                                @else
                                    <a href="{{ $favoritesUrl }}" class="flex h-12 w-12 items-center justify-center rounded-xl bg-[#f6f2ff] text-[#7a5df3] transition hover:bg-[#ece5ff]" aria-label="{{ __('Favorites') }}"><svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20.8 4.6a5.5 5.5 0 00-7.8 0L12 5.6l-1-1a5.5 5.5 0 00-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 000-7.8z"/></svg></a>
                                @endauth
                            </div>
                        </div>

                        <div class="pt-6">
                            <h2 class="font-heading text-[17px] font-bold text-ink">{{ __('About this venue') }}</h2>
                            ...
                            <div class="mt-6 grid gap-3 text-[14px] text-[#4f5579] sm:grid-cols-2 lg:grid-cols-4">
                                @foreach ([$field->type, $field->sport_type, 'Parking', 'Changing Room', 'Shower', 'Toilet', 'Lighting'] as $amenity)
                                    <span class="inline-flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-[#f4f0ff] text-[#6d52ea]"><svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>{{ __($amenity) }}</span>
                                @endforeach
                            </div>
                        </div>
                    </div>
                ...
                <section class="rounded-xl border border-[#ebe7fb] bg-white p-6 shadow-[0_18px_44px_rgba(86,75,165,0.08)] md:p-8">
                    <h2 class="font-heading text-[18px] font-bold text-ink">{{ __('Field Overview') }}</h2>
                    <div class="mt-6 grid gap-6 md:grid-cols-2">
                        @foreach ([['label' => __('Owner'), 'value' => $field->owner->name], ['label' => __('Sport type'), 'value' => __($field->sport_type)], ['label' => __('Field Type'), 'value' => __($field->type)], ['label' => __('Price per Slot'), 'value' => 'Rp ' . number_format((float) $field->price_per_slot, 0, ',', '.')], ['label' => __('Available Days'), 'value' => $availableDays ?: __('No days configured')], ['label' => __('Operating Hours'), 'value' => $operatingHours]] as $detail)
                            <div class="flex gap-4"><span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[#f6f2ff] text-[#8a61ff]"><svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="8"></circle><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 2"/></svg></span><div><p class="text-[12px] font-medium text-copy">{{ $detail['label'] }}</p><p class="mt-1 text-[14px] font-bold text-ink">{{ $detail['value'] }}</p></div></div>
                        @endforeach
                    </div>
                </section>
            </div>
            ...
            <aside class="rounded-xl border border-[#ebe7fb] bg-white p-6 shadow-[0_18px_44px_rgba(86,75,165,0.10)] xl:sticky xl:top-6">
                <h2 class="font-heading text-[18px] font-bold text-ink">{{ __('Date') }}</h2>
                <form method="GET" action="{{ route('fields.show', $field) }}" class="mt-4">
                    <button type="button" id="date-picker-trigger" class="flex w-full cursor-pointer items-center justify-between rounded-lg border border-[#dfdaf4] bg-white px-4 py-4 text-left text-[15px] font-semibold text-[#222746] transition hover:border-[#bfb2ff]">
                        <span class="inline-flex min-w-0 items-center gap-3"><svg class="h-5 w-5 shrink-0 text-[#8b63ff]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"></rect><path stroke-linecap="round" stroke-linejoin="round" d="M16 2v4M8 2v4M3 10h18"/></svg><span id="selected-date-label">{{ $selectedDate->format('m/d/Y') }}</span></span>
                        <svg class="h-4 w-4 shrink-0 text-[#414668]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <input id="booking-date-input" type="date" name="date" value="{{ $selectedDate->toDateString() }}" min="{{ $minBookingDate }}" class="booking-date-input sr-only">
                </form>

                <div class="mt-6 flex items-center justify-between gap-4"><h3 class="font-heading text-[17px] font-bold text-ink">{{ __('Available Time Slots') }}</h3><div class="flex items-center gap-4 text-[12px] text-copy"><span class="inline-flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-[#55d77a]"></span>{{ __('Available') }}</span><span class="inline-flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-[#ff9ca0]"></span>{{ __('Booked') }}</span></div></div>

                @if ($previewSlots->isNotEmpty())
                    <div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                        @foreach ($previewSlots as $slotIndex => $slot)
                            <button type="button" class="slot-button rounded-lg border px-3 py-3 text-center text-[14px] font-bold transition {{ $slot['available'] ? 'border-[#cfeeda] bg-[#edfff1] text-[#008b2f] hover:border-[#83d99a]' : 'cursor-not-allowed border-[#ffd8d8] bg-[#fff0f0] text-[#d20b18]' }}" data-slot-label="{{ $slot['label'] }}" data-slot-index="{{ $slotIndex }}" data-slot-id="{{ $slot['id'] }}" data-slot-start="{{ $slot['start'] }}" data-slot-end="{{ $slot['end'] }}" {{ $slot['available'] ? '' : 'disabled' }}>{{ $slot['label'] }}</button>
                        @endforeach
                    </div>
                @else
                    <div class="mt-4 rounded-lg border border-dashed border-[#dfdaf4] bg-[#faf9ff] px-4 py-6 text-center text-[14px] font-medium text-copy">{{ __('No time slots are available for this date.') }}</div>
                @endif

                <div class="mt-6 border-t border-[#eceaf5] pt-6">
                    <h3 class="font-heading text-[18px] font-bold text-ink">{{ __('Booking Summary') }}</h3>
                    <div class="mt-4 divide-y divide-[#eceaf5] rounded-lg border border-[#eceaf5] px-3 text-[14px]">
                        <div class="flex items-center justify-between gap-4 py-3"><span class="text-copy">{{ __('Venue') }}</span><span class="text-right font-bold text-ink">{{ $field->name }}</span></div>
                        <div class="flex items-center justify-between gap-4 py-3"><span class="text-copy">{{ __('Date') }}</span><span class="font-bold text-ink">{{ $displayDate }}</span></div>
                        <div class="flex items-center justify-between gap-4 py-3"><span class="text-copy">{{ __('Time') }}</span><span id="selected-slot-label" class="font-bold text-ink">{{ $selectedSlotLabel }}</span></div>
                        <div class="flex items-center justify-between gap-4 py-3"><span class="text-copy">{{ __('Duration') }}</span><span id="selected-duration-label" class="font-bold text-ink">{{ $selectedSlot ? __('hour') : '-' }}</span></div>
                        <div class="flex items-center justify-between gap-4 py-3"><span class="font-heading text-[16px] font-bold text-ink">{{ __('Price') }}</span><span id="selected-price-label" class="font-heading text-[22px] font-bold text-[#5a38d6]">Rp {{ $selectedSlot ? number_format($slotPrice, 0, ',', '.') : '0' }}</span></div>
                    </div>

                    @auth
                        <form id="booking-form" method="GET" action="{{ route('bookings.confirm', $field) }}" class="mt-4"><input type="hidden" name="date" value="{{ $selectedDate->toDateString() }}"><div id="selected-slot-inputs"></div><button id="book-now-button" type="submit" {{ $selectedSlot ? '' : 'disabled' }} class="flex w-full items-center justify-center gap-2 rounded-lg bg-[#5a38d6] px-6 py-4 text-[17px] font-bold text-white shadow-[0_14px_28px_rgba(84,66,217,0.26)] transition hover:bg-[#4c2fbd] disabled:cursor-not-allowed disabled:opacity-50"><svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"></rect><path stroke-linecap="round" stroke-linejoin="round" d="M16 2v4M8 2v4M3 10h18"/></svg>{{ __('Book Now') }}</button></form>
                        <p class="mt-4 flex items-center justify-center gap-2 text-[13px] font-medium text-copy"><svg class="h-4 w-4 text-[#5a38d6]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>{{ __('Review your booking and upload proof next') }}</p>
                    @else
                        <form id="booking-form-guest" method="GET" action="{{ route('bookings.confirm', $field) }}" class="mt-4"><input type="hidden" name="date" value="{{ $selectedDate->toDateString() }}"><div id="selected-slot-inputs-guest"></div><button id="book-now-button-guest" type="submit" {{ $selectedSlot ? '' : 'disabled' }} class="flex w-full items-center justify-center gap-2 rounded-lg bg-[#5a38d6] px-6 py-4 text-[17px] font-bold text-white shadow-[0_14px_28px_rgba(84,66,217,0.26)] transition hover:bg-[#4c2fbd] disabled:cursor-not-allowed disabled:opacity-50">{{ __('Book Now') }}</button></form>
                        <p class="mt-4 text-center text-[13px] font-medium text-copy">{{ __('Sign in to continue booking.') }}</p>
                    @endauth
                </div>
            </aside>
        </section>
    ...
    <script>
        ...
        const hourText = @json(__('hour'));
        const hoursText = @json(__('hours'));
        let selectedIndexes = [];

        if (datePickerTrigger && bookingDateInput instanceof HTMLInputElement) {
            datePickerTrigger.addEventListener('click', () => {
                if (typeof bookingDateInput.showPicker === 'function') { bookingDateInput.showPicker(); return; }
        ...
        const updateBookingSummary = () => {
            ...
            slots.forEach((slotButton) => slotButton.classList.add('ring-2', 'ring-[#7d68f2]', 'border-[#7d68f2]', 'bg-[#e6dcff]', 'text-[#4b2fc4]'));
            if (slots.length === 0) {
                if (selectedSlotLabel) selectedSlotLabel.textContent = noAvailableSlotsText;
                if (selectedDurationLabel) selectedDurationLabel.textContent = '-';
                if (selectedPriceLabel) selectedPriceLabel.textContent = 'Rp 0';
                if (bookNowButton instanceof HTMLButtonElement) bookNowButton.disabled = true;
                if (bookNowButtonGuest instanceof HTMLButtonElement) bookNowButtonGuest.disabled = true;
                if (selectedSlotInputs) selectedSlotInputs.innerHTML = '';
                if (selectedSlotInputsGuest) selectedSlotInputsGuest.innerHTML = '';
                return;
            }
            const firstSlot = slots[0];
            const lastSlot = slots[slots.length - 1];
            const durationLabel = `${slots.length} ${slots.length === 1 ? hourText : hoursText}`;
            if (selectedSlotLabel) selectedSlotLabel.textContent = `${firstSlot.dataset.slotStart} - ${lastSlot.dataset.slotEnd}`;
            if (selectedDurationLabel) selectedDurationLabel.textContent = durationLabel;
            if (selectedPriceLabel) selectedPriceLabel.textContent = formatRupiah(slots.length * slotPrice);
            if (bookNowButton instanceof HTMLButtonElement) bookNowButton.disabled = false;
            if (bookNowButtonGuest instanceof HTMLButtonElement) bookNowButtonGuest.disabled = false;
            const slotInputsHtml = slots.map((slotButton) => `<input type="hidden" name="slot_ids[]" value="${slotButton.dataset.slotId}">`).join('');
            if (selectedSlotInputs) selectedSlotInputs.innerHTML = slotInputsHtml;
            if (selectedSlotInputsGuest) selectedSlotInputsGuest.innerHTML = slotInputsHtml;
        };
        ...
        if (firstAvailableSlot) { selectedIndexes = [Number(firstAvailableSlot.dataset.slotIndex)]; updateBookingSummary(); } else { updateBookingSummary(); }
    </script>
</body>
</html>
