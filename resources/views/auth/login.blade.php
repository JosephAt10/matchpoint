...
<head>
    ...
    <title>{{ __('Login') }} - MatchPoint</title>
...
<body class="min-h-screen bg-white text-[#22263d]">
    <main class="grid min-h-screen lg:grid-cols-2">
        <section class="relative hidden overflow-hidden lg:block">
            <img src="{{ asset('landing/login-page-image.png') }}" alt="{{ __('MatchPoint multi-sport login') }}" class="absolute inset-0 h-full w-full object-cover object-left opacity-42">
        ...
        <section class="flex items-center justify-center bg-[radial-gradient(circle_at_top_right,_rgba(138,118,255,0.14),_transparent_22%),linear-gradient(180deg,_#ffffff_0%,_#faf8ff_100%)] px-6 py-12 lg:px-10">
            <div class="w-full max-w-[540px] rounded-[2rem] border border-softBorder bg-white p-7 shadow-auth md:p-10">
                ...
                <form action="{{ route('login.store') }}" method="POST" novalidate class="mt-10 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border {{ $errors->has('password') || $errors->has('email') ? 'border-rose-400' : 'border-softBorder' }} bg-white px-4 py-4">
                            ...
                            <input id="password" type="password" name="password" placeholder="{{ __('Password') }}" class="w-full border-0 p-0 text-[19px] text-[#363b55] outline-none placeholder:text-[#9d98ba] md:text-[16px]" required>
                            <button type="button" id="toggle-password" class="text-[#9994b9] transition hover:text-[#6d63ea]" aria-label="{{ __('Password') }}" aria-pressed="false">
                    ...
                    <div class="flex flex-col gap-4 text-[18px] text-copy md:flex-row md:items-center md:justify-between md:text-[16px]">
                        ...
                        <a href="#" class="font-medium text-indigo hover:text-indigoDark">{{ __('Forgot password?') }}</a>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <span class="text-[18px] text-copy md:text-[16px]">{{ __('or') }}</span>
                        <div class="h-px flex-1 bg-softBorder"></div>
                    </div>
                ...
                <p class="mt-8 text-center text-[18px] text-copy md:text-[16px]">
                    ...
                    <a href="{{ route('register') }}" class="font-semibold text-indigo hover:text-indigoDark">{{ __('Sign Up') }}</a>
                </p>
            </div>
        </section>
    </main>
