    ...
    <header class="border-b border-[#e4defc] bg-white/90 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
            ...
            <nav class="flex items-center gap-3 text-sm">
                <a href="{{ route('fields.index') }}" class="rounded-full px-4 py-2 text-[#4b5070] transition hover:bg-[#f3efff]">{{ __('Browse Fields') }}</a>
                @include('partials.locale-switcher')

                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-full px-4 py-2 text-[#4b5070] transition hover:bg-[#f3efff]">{{ __('Dashboard') }}</a>
                    <a href="{{ route('profile.edit') }}" class="rounded-full px-4 py-2 text-[#4b5070] transition hover:bg-[#f3efff]">{{ __('Profile') }}</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="rounded-full px-4 py-2 text-[#4b5070] transition hover:bg-[#f3efff]">{{ __('Login') }}</a>
