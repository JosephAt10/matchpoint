    ...
    <div class="grid min-h-[calc(100vh-85px)] lg:grid-cols-[292px_minmax(0,1fr)]">
        <aside class="sidebar-shell border-r border-[#dde5f3] shadow-[1px_0_0_rgba(255,255,255,0.04)] px-6 py-8">
            ...
            <nav class="space-y-2">
                ...
                <a href="{{ route('matches.index') }}" class="sidebar-nav-link flex items-center gap-4 rounded-2xl px-5 py-4 font-medium transition"><span class="h-6 w-6">{!! $iconSvg('calendar') !!}</span><span>{{ __('Public Matches') }}</span></a>
                <a href="{{ route('matches.my') }}" class="sidebar-nav-link flex items-center gap-4 rounded-2xl px-5 py-4 font-medium transition"><span class="h-6 w-6">{!! $iconSvg('calendar') !!}</span><span>{{ __('My Matches') }}</span></a>
